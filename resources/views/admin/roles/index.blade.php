...
@section('content')
    ...
    <div class="card card-teal card-outline">
        <div class="card-header text-right">
            <a href="{{route('role.create')}}" class="btn btn-default"><i class="fas fa-plus mr-2"></i> Cadastrar Perfil</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                ...
                <table id="example1" class="table table-bordered table-striped projects">
                    ...
                    <tbody> 
                        ...
                        <tr>                            
                            ...
                            <td>{{$role->name}}</td>
                            <td>
                                <a href="{{route('role.edit',['role' => $role->id])}}" class="btn btn-xs btn-default"><i class="fas fa-pen"></i></a>
                                <a class="btn btn-xs btn-outline-success" href="{{ route('role.permissions', ['role' => $role->id]) }}">Permissões</a>
                ...
                $.ajax({
                    ...
                    success:function(data) {
                        if(data.error){
                            ...
                            $('#id_role').val(data.id);
                            $('#frm').prop('action','{{ route('role.deleteon') }}');
                        }else{
                            $('#frm').prop('action','{{ route('role.deleteon') }}');
                        }
                    }
